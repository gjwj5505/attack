...
let analysis_sem ?(init_cenv = Environment.empty) (prog : Cmd.lbl_t) :
  ...
  let init_mem =
    Environment.VarMap.fold
      (fun x cval mem -> Abs_mem.add x (Itv.singleton cval) mem)
      init_cenv Abs_mem.empty
  in
  sem := Cmd.Lbl_map.add start init_mem !sem;
...
let string_of_aval = Abs_val.string_of_t

let string_of_aenv = Abs_mem.string_of_t

let string_of_analysis_sem sem _pgm = Abs_sem.string_of_t sem
