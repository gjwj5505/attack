...
module Abs_mem = struct
  ...
  let find loc mem =
    ...
    | Mem m -> (
      ...
      | Some v -> v
      | None -> Itv.singleton 0 (* default value for uninitialized variables *)
    )
  ...
  let widen m_old m_new =
    ...
    | Mem old, Mem new_ ->
        ...
        Mem (Loc.Map.merge (fun _ v_old_opt v_new_opt ->
  ...
  (* module Set = struct
    include Set.Make (struct
      ...
      let compare = Loc.Map.compare Abs_val.compare
    end)
  ...
  let string_of_t sem =
    Cmd.Lbl_map.fold
      (fun lbl mem acc ->
        ...
        acc ^ semicolon ^ Cmd.Lbl_map.string_of_key lbl ^ " |-> " ^ Abs_mem.string_of_t mem)
      sem
      ""
end
