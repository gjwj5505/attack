...
let value_to_json v =
  `Assoc [
    ("itv", `String (Itv.to_string (Val.itv_of_val v)));
    ("raw", `String (Val.to_string v))
  ]

let mem_to_json mem =
  `Assoc [
    ("is_bot", `Bool (Mem.eq mem Mem.bot));
    ("bindings",
     `List
       (Mem.foldi
          (fun loc value acc ->
            `Assoc [
              ("loc", `String (Loc.to_string loc));
...
let table_to_json table =
  `List
    (Table.foldi
       (fun node mem acc ->
         `Assoc [
           ("node", `String (InterCfg.Node.to_string node));
           ("mem", mem_to_json mem)
         ] :: acc)
       table []
     |> List.rev)

let alarm_summary_to_json alarms =
  `Assoc [
    ("total", `Int (List.length alarms));
    ("proven", `Int (List.length (Report.get alarms Proven)));
    ("unproven", `Int (List.length (Report.get alarms UnProven)));
    ("bot", `Int (List.length (Report.get alarms BotAlarm)))
  ]

let main_exit_node global =
  let fallback = InterCfg.exitof global.icfg "main" in
  try
    let main_cfg = InterCfg.cfgof global.icfg "main" in
    match IntraCfg.pred (IntraCfg.exitof main_cfg) main_cfg with
    | pred :: _ -> InterCfg.Node.make "main" pred
    | [] -> fallback
  with _ -> fallback

let dump_json global inputof outputof alarms =
  if !Options.json_dump <> "" then
    let json =
      `Assoc [
          ("file", `String global.file.Cil.fileName);
          ("analysis", `String "itv");
          ("main_exit_node",
           `String (InterCfg.Node.to_string (main_exit_node global)));
          ("alarms", alarm_summary_to_json alarms);
          ("input", table_to_json inputof);
          ("output", table_to_json outputof)
        ]
    in
    let oc = open_out !Options.json_dump in
    try
...
= fun global ->
  ...
  |> StepManager.stepf true "Generate Alarm Report" (fun (global,inputof,outputof) ->
      let alarms = inspect_alarm global spec inputof in
      dump_json global inputof outputof alarms;
