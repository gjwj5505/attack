...
and check_stree tree =
  ...
  | STreeReturnNone (_mem, stmt, _out_mem, control) ->
      ...
      >>= fun () -> check_control "S-ReturnNone control" ReturnVoid control
...
and check_btree tree =
  match tree with
  | BTreeSeq (strees, (mem, block, out_mem, control)) ->
      check_list check_stree strees >>= fun () ->
      check_block_flow mem strees >>= fun () ->
      let expected_out =
